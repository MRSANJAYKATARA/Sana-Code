...
# 🚀 Sana Code (Powered by SanjayXLegacy)
...
> [!IMPORTANT]
> Sana Code is an experimental alpha for ARM64 Android devices. Its PRoot environment is not a hardened security sandbox. Open only projects you own or trust, and do not use the app for sensitive or untrusted code.

## Overview

Sana Code brings a professional-grade, autonomous AI coding workspace to an Android phone without requiring a PC. The Android interface stays beginner-friendly while an app-private Ubuntu environment runs the command-line tools behind it.
...
```bash
git clone https://github.com/MRSANJAYKATARA/Sana-Code.git
cd Sana-Code
