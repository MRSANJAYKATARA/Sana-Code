...
class ClaudeRuntimeBridge(
    ...
    private fun buildContextPrompt(currentPrompt: String, history: List<ChatMessage>, guestWorkspacePath: String, projectKind: ProjectKind): String {
        ...
        val priorMessages = history
            .filter { msg ->
                (msg.fromUser || !msg.text.startsWith("Hi! Tell me") && !msg.text.startsWith("🚀 Sana Code activated")) &&
        ...
        sb.appendLine("<sana_code_system>")
        sb.appendLine("You are SANA CODE — an autonomous AI development agent running inside Sana Code on Android.")
        sb.appendLine("Developer: Sanjay Katara | Powered By: SanjayXLegacy")
        sb.appendLine("YouTube Channel: https://youtube.com/@sanjayxlegacy?si=WdwPmoIqYRZhQ-cV")
        sb.appendLine("Mission: Convert any user idea into a fully production-ready, market-deployable digital product using only an Android phone.")
        sb.appendLine("Operating Principles:")
        sb.appendLine("1. Never ask the user to switch to PC. Solve everything on-device.")
        sb.appendLine("2. Never produce half-baked code, dummy logic, or placeholder hacks. Every output must be ship-ready.")
        sb.appendLine("3. Format responses using headers: 🎯 TASK, 📋 PLAN, ⚙️ TECH, 📁 STRUCTURE, 🛠️ CODE, 🧪 TESTS, 📦 BUILD, 📝 DOCS, ✅ CHECKLIST.")
        sb.appendLine("</sana_code_system>")
        sb.appendLine()
